...
@section('title', 'Inventarios ' . ($productoNombre ?? 'Lubricantes'))

@section('content')
    <div class="pastel-section mb-3">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
            <div>
                <h4 class="mb-0 text-warning-emphasis fw-bold">INVENTARIOS {{ $productoNombre ?? 'LUBRICANTES' }}</h4>
                <small class="text-muted">Formato de control por planilla con kardex al costo promedio y ventas.</small>
            </div>
            <button type="submit" form="inventario-lubricante-form" class="btn btn-primary btn-sm">Guardar inventario</button>
        </div>
    </div>
    ...
    <form id="inventario-lubricante-form" method="POST"
        action="{{ route($formRoute ?? 'inventarios-lubricantes.store') }}">
        @csrf
        <input type="hidden" name="producto" value="{{ old('producto', $productoNombre ?? 'MOBIL SUPER 20W50') }}">
        ...
        <x-erp-card title="INVENTARIOS {{ $productoNombre ?? 'LUBRICANTES' }}">
            <div class="table-responsive">
                <table class="table table-bordered table-sm mb-0 align-middle" id="inventario-lubricante-table">
                    <thead>
                        <tr style="background-color:#000; color:#f5f2e9; letter-spacing:.08rem; font-weight:800;">
                            <th colspan="14" class="text-center fs-4">
                                INVENTARIOS&nbsp;&nbsp;&nbsp;&nbsp;{{ $productoTitulo ?? '1 - MOBIL SUPER 20W50' }}</th>
                        </tr>
                        <tr style="background-color:#f6e7d4;">
                            <th colspan="2" class="text-center align-middle">PLANILLA</th>
                            <th colspan="5" class="text-center align-middle">UNIDADES</th>
                    ...
                    <tbody id="inventario-lubricante-body">
                        <tr style="background-color:#eef1f5; font-weight:700;">
                            <td colspan="6" class="text-uppercase">Saldo anterior</td>
                            <td>
                                <input type="text" name="saldo_anterior_unidades" id="saldo-anterior-unidades"
                                    class="form-control form-control-sm text-end" inputmode="decimal"
                                    value="{{ old('saldo_anterior_unidades', '0') }}">
                            </td>
                            <td></td>
                            <td></td>
                            <td>
                        ...
                        @foreach ($rows as $index => $row)
                            <tr data-index="{{ $index }}">
                                ...
                                <td>
                                    <input type="text" name="rows[{{ $index }}][fc_no]"
                                        class="form-control form-control-sm" value="{{ $row['fc_no'] ?? '' }}">
                                </td>
                                <td>
                                    <select name="rows[{{ $index }}][proveedor_id]"
                                        class="form-select form-select-sm proveedor-select">
                                        <option value="">Seleccione proveedor</option>
                                        @foreach ($proveedores ?? collect() as $proveedor)
                                            <option value="{{ $proveedor->id }}" @selected((string) ($row['proveedor_id'] ?? '') === (string) $proveedor->id || ($row['proveedor'] ?? '') === $proveedor->name)>
                                                {{ $proveedor->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="hidden" name="rows[{{ $index }}][proveedor]"
                                        value="{{ $row['proveedor'] ?? '' }}">
                                </td>
                                <td>
                                    <input type="text" name="rows[{{ $index }}][entradas_unidades]"
                                        class="form-control form-control-sm text-end entradas-unidades-input"
                                        inputmode="decimal" value="{{ $row['entradas_unidades'] ?? '' }}">
                                </td>
                                <td>
                                    <input type="text" name="rows[{{ $index }}][salidas_unidades]"
                                        class="form-control form-control-sm text-end salidas-unidades-input"
                                        inputmode="decimal" value="{{ $row['salidas_unidades'] ?? '' }}">
                                </td>
                                <td>
                                    <input type="text"
                                        class="form-control form-control-sm text-end saldo-unidades-output" readonly>
                                </td>
                                <td>
            ...
            <div class="p-3 d-flex justify-content-end">
                <button type="button" id="add-inventario-lubricante-row" class="btn btn-sm btn-outline-primary">+
                    Agregar fila</button>
            </div>
        </x-erp-card>
    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const tbody = document.getElementById('inventario-lubricante-body');
            const addBtn = document.getElementById('add-inventario-lubricante-row');
            const proveedorOptionsHtml = document.getElementById('proveedores-options-template').innerHTML;
            const saldoAnteriorUnidadesInput = document.getElementById('saldo-anterior-unidades');
            ...
            function updateTotals(acc) {
                ...
                document.getElementById('total-saldo').textContent = formatNumber(acc.ultimoSaldoUnidades, 3);
            ...
            function updateRows() {
                let saldoAnteriorUnidades = parseNumber(saldoAnteriorUnidadesInput.value);
                ...
                if (saldoAnteriorPromedio <= 0 && saldoAnteriorUnidades > 0) {
                    saldoAnteriorPromedio = saldoAnteriorValor / saldoAnteriorUnidades;
                    setFormattedValue(saldoAnteriorPromedioInput, saldoAnteriorPromedio, 0);
                }

                const totals = {
                    totalSalidas: 0,
                    ultimoSaldoUnidades: saldoAnteriorUnidades,
                ...
                getRows().forEach(row => {
                    const entradasInput = row.querySelector('.entradas-unidades-input');
                    const salidasInput = row.querySelector('.salidas-unidades-input');
                    const saldoOutput = row.querySelector('.saldo-unidades-output');
                    ...
                    const saldoUnidades = saldoAnteriorUnidades + entradas - salidas;
                    const valorSaldo = saldoAnteriorValor + valorEntradas - valorSalidas;
                    const promedio = saldoUnidades > 0 ? valorSaldo / saldoUnidades : 0;
                    const vrVenta = salidas * precioVenta;

                    setFormattedValue(saldoOutput, saldoUnidades, 3);
                    ...
                    totals.ultimoSaldoUnidades = saldoUnidades;
            ...
            function attachRowEvents(row) {
                row.querySelectorAll(
                        '.entradas-unidades-input, .salidas-unidades-input, .valor-entradas-input, .precio-venta-input'
                    )
                    .forEach(input => {
                        input.addEventListener('input', updateRows);
                        input.addEventListener('change', updateRows);
                        input.addEventListener('blur', function() {
                            const decimals = input.classList.contains('entradas-unidades-input') ||
                                input.classList.contains('salidas-unidades-input') ? 3 : 0;
            ...
            function createRow(index) {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <input type="text" name="rows[${index}][fc_no]" class="form-control form-control-sm">
                    </td>
                    <td>
                        <select name="rows[${index}][proveedor_id]" class="form-select form-select-sm proveedor-select">
                            ${proveedorOptionsHtml}
                        </select>
                        <input type="hidden" name="rows[${index}][proveedor]" value="">
                    </td>
                    <td>
                        <input type="text" name="rows[${index}][entradas_unidades]" class="form-control form-control-sm text-end entradas-unidades-input" inputmode="decimal">
                    </td>
                    <td>
                        <input type="text" name="rows[${index}][salidas_unidades]" class="form-control form-control-sm text-end salidas-unidades-input" inputmode="decimal">
                    </td>
                    <td>
                        <input type="text" class="form-control form-control-sm text-end saldo-unidades-output" readonly>
                    </td>
                    <td>
            ...
            [saldoAnteriorUnidadesInput, saldoAnteriorValorInput, saldoAnteriorPromedioInput,
            ...
            .forEach(input => {
                ...
                input.addEventListener('change', updateRows);
                input.addEventListener('blur', function() {
                    const decimals = input.id === 'saldo-anterior-unidades' ? 3 : 0;
                    setFormattedValue(input, parseNumber(input.value), decimals);
                });
            });
